"""Cargo command execution and error handling behaviour."""
...
def test_cargo_commands_invoke_runner(
    run_publish_check_module: ModuleType,
    mock_cargo_runner: list[tuple[str, Path, tuple[str, ...], int]],
    ...
    assert mock_cargo_runner == [
        (crate, workspace, tuple(expected_command), timeout)
    ]
