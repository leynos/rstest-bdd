...
from typing import Annotated, Callable, Optional, Protocol, Sequence
...
DEFAULT_LIVE_COMMANDS: tuple[Command, ...] = (
    ("cargo", "publish", "--dry-run"),
    ("cargo", "publish"),
)

LIVE_COMMAND_OVERRIDES: dict[str, tuple[Command, ...]] = {
    "cargo-bdd": (
...
def run_cargo_command(
    ...
    *,
    ...
    command
        Command arguments, expressed as any :class:`~collections.abc.Sequence`
        that **must** begin with ``cargo``.
...
def _commands_for_crate(crate: str) -> tuple[Command, ...]:
    """Return the publish command sequence configured for ``crate``."""

    return LIVE_COMMAND_OVERRIDES.get(crate, DEFAULT_LIVE_COMMANDS)
...
def publish_crate_commands(
    ...
    *,
    ...
    commands = _commands_for_crate(crate)
    if not commands:
        raise SystemExit(f"missing live publish commands for {crate!r}")
