...
def run_cargo(crate: str, workspace_root: Path, args: list[str]) -> None:
    crate_dir = workspace_root / "crates" / crate
    env = dict(os.environ)
    env["CARGO_HOME"] = str(workspace_root / ".cargo-home")
    subprocess.run(["cargo", *args], check=True, cwd=crate_dir, env=env)


def package_crate(crate: str, workspace_root: Path) -> None:
    run_cargo(crate, workspace_root, ["package", "--allow-dirty", "--no-verify"])


def check_crate(crate: str, workspace_root: Path) -> None:
    run_cargo(crate, workspace_root, ["check", "--all-features"])
