...
- **Use consistent spelling and grammar.** Comments must use en-GB-oxendict
  ...
  references to external APIs. This convention is automatically enforced for
  Markdown by the `typos` spelling gate, which runs as part of
  `make markdownlint` / `make spellcheck`.
...
- Validate Markdown files using `make markdownlint`. This target runs
  markdownlint-cli2 and then `make spellcheck`, which enforces en-GB-oxendict
  (Oxford) spelling via `typos`.
- The spelling configuration lives in `typos.toml`, which is GENERATED by
  `scripts/generate_typos_config.py`. Never edit `typos.toml` by hand — edit
  the generator's `STEMS`, `EXTRA_ACCEPTED_WORDS`, or `extend-ignore-re` lists
  and regenerate with `uv run scripts/generate_typos_config.py`.
- Quoted API names and code identifiers keep their upstream (often US)
  spelling; wrap them in backticks so the gate ignores them, rather than adding
...
on the `PATH`. The fragment is passed explicitly with `--config`, so release,
...
The targets use `DEV_FAST_TOOLCHAIN ?= nightly` by default; set that Make
variable when a different nightly toolchain is required.
