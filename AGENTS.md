...
- Validate Markdown files using `make markdownlint`. This target runs
  markdownlint-cli2 and then `make spellcheck`, which enforces
  en-GB-oxendict (Oxford) spelling via `typos`.
- The spelling configuration lives in `typos.toml`, which is GENERATED by
  `scripts/generate_typos_config.py`. Never edit `typos.toml` by hand —
  edit the generator's `STEMS`, `EXTRA_ACCEPTED_WORDS`, or
  `extend-ignore-re` lists and regenerate with
  `uv run scripts/generate_typos_config.py`.
- Quoted API names and code identifiers keep their upstream (often US)
  spelling; wrap them in backticks so the gate ignores them, rather than
