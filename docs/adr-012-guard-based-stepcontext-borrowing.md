# Architectural decision record (ADR) 012: guard-based `StepContext` borrowing
...
- The generated test constructs a fresh `StepContext` and fresh owned fixture
  cells per scenario run. This framework-owned storage guarantee does not alter
  `rstest` fixture scopes, including intentional `#[once]` sharing.
