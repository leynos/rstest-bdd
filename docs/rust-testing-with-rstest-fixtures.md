...
fixture. Frameworks such as `rstest-bdd` will inject a fixture automatically
when the parameter name matches the fixture, making the attribute optional in
those cases. The `#[from(original_fixture_name)]` attribute on an argument
allows renaming. This is particularly useful when destructuring the result of a
fixture.
