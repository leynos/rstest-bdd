...
Scenario bodies may be fallible. Declare the test function to return
`Result<(), E>` (or `StepResult<(), E>`) and use `?` to propagate errors from
helpers. When the body returns `Err`, the test fails and the scenario is not
recorded as passed.
...
- **Step pattern registry (on save)**: Compiles the indexed step patterns with
  ...
  - **API note (embedding)**: `StepDefinitionRegistry::{steps_for_keyword, steps_for_file}` returns `Arc<CompiledStepDefinition>` entries so the
...
1. Place the cursor on a Rust function annotated with `#[given]`, `#[when]`, or
   `#[then]`.
1. Invoke "Go to Definition" (typically F12 or Ctrl+Click in most editors).
1. The editor navigates to all matching steps in `.feature` files.
...
1. Invoke "Go to Implementation" (typically Ctrl+F12 or a similar keybinding in
   most editors).
1. The editor navigates to all matching Rust step functions.
