...
```toml
[test-groups] stateful-gpui = { max-threads = 1 }

[[profile.default.overrides]] filter = 'test(stateful_gpui::)' test-group =
'stateful-gpui'
```
...
```toml
[dev-dependencies] serial_test = { version = "…", features = ["file_locks"] }
```
...
```gherkin
@allow_skipped Scenario: Behaviour pending external contract Given a dependent
...
let outcome = StepExecution::skipped(Some("maintenance pending".into())); let
message = assert_step_skipped!(outcome, message = "maintenance");
...
```rust,no_run
use rstest::fixture; use rstest_bdd_macros::{given, scenarios};
...
```rust,no_run
use rstest_bdd_macros::{given, scenario, then, when}; use rstest::fixture;
...
```rust,no_run
use rstest::fixture; use rstest_bdd_macros::{given, scenarios, when};
...
// Note: runtime = "tokio-current-thread" is deprecated; prefer explicit
harness form scenarios!(
...
```rust,no_run
use rstest_bdd::async_step::sync_to_async; use rstest_bdd::{StepContext,
StepError, StepExecution, StepFuture};
...
```rust,no_run
use rstest_bdd::async_step::sync_to_async; use rstest_bdd::{
...
let harness = MyHarness; assert_eq!(harness.run(request).expect("harness should
not fail"), "ok");
```
...
let attrs = DefaultAttributePolicy::test_attributes(); assert_eq!(attrs.len(),
1); assert_eq!(attrs[0].render(), "#[rstest::rstest]");
```
...
```toml
[dev-dependencies] rstest-bdd-macros = { version = "0.6.0-beta3", features = ["
...
```rust,no_run
use rstest_bdd::datatable::{self, DataTableError, Rows}; use
rstest_bdd_macros::DataTableRow;
...
```rust,no_run
use rstest_bdd::datatable::{DataTableError, Rows}; use
rstest_bdd_macros::{DataTable, DataTableRow};
...
let ok: Result<(), &str> = Ok(()); assert_step_ok!(ok);

let err: Result<(), &str> = Err("boom"); let e = assert_step_err!(err, "boom");
assert_eq!(e, "boom");
```
...
```gherkin
# language: es
Característica: Control de stock Escenario: Añadir una calabaza
...
```toml
[dependencies] rstest-bdd = "0.6.0-beta3" i18n-embed = { version = "0.16",
features = ["fluent-system", "desktop-requester"] } unic-langid = "0.9"
```
...
use rstest_bdd::select_localizations; use unic_langid::langid;
...
```rust,no_run
let mut buffer = Vec::new(); rstest_bdd::reporting::json::write_snapshot(&mut
buffer)?;
```
...
```rust,no_run
let mut xml = String::new(); rstest_bdd::reporting::junit::write_snapshot(&mut
xml)?;
```
...
```bash
rstest-bdd-lsp --log-level debug --debounce-ms 100 --workspace-root
/path/to/project
```
...
```lua
local lspconfig = require('lspconfig') local configs =
require('lspconfig.configs')

if not configs.rstest_bdd then configs.rstest_bdd = {
    default_config = {
      cmd = { 'rstest-bdd-lsp' },
