...
- **Use placeholders for dynamic values.** Pattern strings may include
  ...
  literal braces with `{{` and `}}`. Use `\\` to match a single backslash. Any
  other backslash escapes are treated literally, so `\d` matches the
  two-character sequence `\d`. Nested braces inside placeholders are not
