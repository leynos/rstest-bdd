...
- **No `.expect()` or `.unwrap()` outside `#[cfg(test)]`.** Whitaker's
...
- **Risk:** file-length ceiling breached. `classify.rs` is 382/400 lines and
...
- Observation: three source files this plan touches are within 20 lines of the
  400-line ceiling. Evidence: `wc -l` gives 382 for `classify.rs`, 384 for
...
- `extract_flag_attribute(arg, attr_name)` in `classify.rs` (line 107). Strips
  every attribute whose path is `attr_name` from a parameter, returning whether
  one was present, and erroring on the argued form (`#[foo(1)]`) or a
...
- `DATATABLE_WITH_FROM_ERROR` in `classify.rs` (line 50) plus its check at line
  315.
...
Create
`crates/rstest-bdd-macros/src/codegen/wrapper/args/classify/harness_context.rs`
...
Declare the new module from `classify.rs`. That file has 18 lines of head-room;
a `mod harness_context;` line plus a `pub(super) use` re-export fits
...
`#[from(rstest_bdd_harness_context)]`" (around line 1029), replace the example
...
- `.../args/classify/harness_context.rs`
