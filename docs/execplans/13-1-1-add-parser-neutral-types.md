...
- [ ] EP-M5: documentation, snapshots, and the full gate.
  ...
  - [x] (2026-09-26) **The resolution was verified by running the stop hook
    itself, with a non-vacuity control.** Committing and pushing `c1d94cc0` was
    necessary but not sufficient evidence, because a hook that passes could
    simply be a hook that skipped its checks. Measured directly, on a tree
    confirmed clean (0 entries) with 0 unpushed commits:

    ```plaintext
    post-turn-quality-stop-hook  →  exit 0, stdout 0 bytes, post-state clean
    post-turn-quality-stop-hook  →  exit 0, stdout 0 bytes, post-state clean   (2nd run)
    ```

    Two consecutive passes, and the file the hook rewrites is still committed
    afterwards rather than newly dirtied. The control then establishes that the
    gate is live rather than inert: introducing one untracked scratch file and
    re-running produced a **230-byte block payload**
    (`"Please commit outstanding changes before ending this turn."`) with exit
    0 — the hook reports its decision in stdout, not in its exit status. So the
