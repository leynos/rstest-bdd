...
framework resources to step execution (for example GPUI `TestAppContext` or Bevy
`World`) in a type-safe, reusable way.

After this work:

- A new architectural decision record (ADR) evaluates three approaches and
  selects one for both GPUI and Bevy: thread-local convention, associated
  `Context` type on `HarnessAdapter`, or a `StepContext` extension trait.
...
- Keep ADR-005a intent intact: framework specifics stay in opt-in harness
...
- Risk: changing `HarnessAdapter` and `ScenarioRunRequest` generics could ripple
  through macros, harness crates, and tests. Severity: high. Likelihood:
  medium. Mitigation: stage the change with compile-first checkpoints and
  focused migration commits.

- Risk: thread-local convention can hide coupling and make test ordering or
  nested harness behaviour fragile. Severity: medium. Likelihood: medium.
...
- Risk: a `StepContext` extension trait may over-couple harness internals to
  runtime context layout. Severity: medium. Likelihood: medium. Mitigation:
  evaluate trait ownership, object safety, and long-term evolution costs in ADR
  option analysis.

- Risk: roadmap text references "ADR-006" for context injection, but
  `docs/adr-006-fallible-scenario-functions.md` already exists. Severity:
  medium. Likelihood: high. Mitigation: assign the next available ADR number
  and update references where needed so numbering is unambiguous.

- Risk: documentation drift between roadmap, design doc, users guide, and ADR.
  Severity: medium. Likelihood: medium. Mitigation: update all docs in one
  stage and verify cross-links in review.
...
- [x] (2026-02-26) Reviewed ADR-005a and current design/user docs for harness
      adapters and fixture injection semantics.
- [x] (2026-02-26) Drafted this ExecPlan.
- [x] (2026-03-03) Stage A: baseline and ADR framing.
- [x] (2026-03-03) Stage B: prototype and evaluate the three injection
      approaches.
- [x] (2026-03-03) Stage C: implement selected mechanism in harness core types.
- [x] (2026-03-03) Stage D: adapt macro/runtime integration and harness plugin
      crates.
- [x] (2026-03-03) Stage E: add unit and behavioural validation.
- [x] (2026-03-03) Stage F: update ADR/design doc/users guide and mark roadmap
      item done.
- [x] (2026-03-03) Stage G: run quality gates and capture logs.
...
- Observation: roadmap phase 9.5.1 still names the context-injection ADR as
  ...
  Evidence: `docs/roadmap.md` and
  `docs/adr-006-fallible-scenario-functions.md`. Impact: this plan must
  allocate a non-conflicting ADR number for fixture injection and normalize
  references.
...
- Observation: macro harness delegation currently builds `StepContext` inside an
  ...
  fixture injection. Evidence:
  `crates/rstest-bdd-macros/src/codegen/scenario/runtime/harness.rs`. Impact:
  any selected mechanism must shift context handoff visibility to the harness
  boundary.
...
- Decision: treat 9.4.1 as a design-plus-delivery milestone, not ADR-only.
  ...
  and behavioural validation. Date/Author: 2026-02-26 / Codex.

- Decision: include an explicit prototyping stage for all three options before
  selecting the final approach. Rationale: the task is architecture-sensitive
  and impacts multiple crates. Fast prototyping de-risks API lock-in.
  Date/Author: 2026-02-26 / Codex.

- Decision: choose associated `Context` on `HarnessAdapter` and publish
  ADR-007. Rationale: this provides explicit, typed context handoff for GPUI
  and Bevy without thread-local coupling or `StepContext` layering leakage.
...
- Added `docs/adr-007-harness-context-injection.md` evaluating all required
  options and selecting associated `Context`.
- Implemented associated context in core harness types:
  `HarnessAdapter::Context`, `ScenarioRunner<'a, C, T>`, and
  `ScenarioRunRequest<'a, C, T>`.
- Migrated `StdHarness`, `TokioHarness`, macro harness codegen, and scenario
  harness integration tests to the new interface.
- Added context-focused validation in both unit and behavioural suites
  (including non-unit context harness probes).
- Updated `docs/rstest-bdd-design.md` and `docs/users-guide.md` with the new
  usage contract.
- Marked roadmap entry 9.4.1 as complete and normalized the roadmap reference
  from ADR-006 to ADR-007 for this topic.

Retrospective:

- The associated-context migration was low-risk once runner/request generics
  were updated first and call sites migrated immediately after.
- Macro generation required only one focused update because the harness-bound
  closure is isolated in `runtime/harness.rs`.
- Trybuild snapshot drift was limited to line-number shifts in
  `scenario_harness_not_default.stderr` after adding `type Context = ();`.
...
- [ADR-005a][adr-005a] Existing harness architecture ADR.
...
```plaintext
/tmp/9-4-1-make-fmt.log
/tmp/9-4-1-check-fmt.log
/tmp/9-4-1-lint.log
/tmp/9-4-1-test.log
/tmp/9-4-1-markdownlint.log
/tmp/9-4-1-nixie.log
/tmp/9-4-1-trybuild.log
/tmp/9-4-1-targeted-tests.log
/tmp/9-4-1-final-check-fmt.log
/tmp/9-4-1-final-lint.log
/tmp/9-4-1-final-test.log
/tmp/9-4-1-final-markdownlint.log
/tmp/9-4-1-final-nixie.log
```

Primary implementation artefact set:
...
Related request/runner threading:
...
Initial draft created on 2026-02-26 for roadmap item 9.4.1 and completed on
2026-03-03.
