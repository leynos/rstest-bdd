...
For other non-unit returns, the generated wrapper asks the hidden runtime bridge
to select between a concrete `Result<T, E>` and an ordinary value. This makes a
local alias behave as its underlying type without treating wrappers such as
`Box<Result<..>>` or `Option<Result<..>>` as fallible.
