...
`AttributePolicy::test_attributes()` for an arbitrary user-provided type
during expansion. Instead, the macro emits compile-time const trait-bound
assertions:
...
attributes such as `#[tokio::test]`. The rationale is that the attribute
policy is the designated extension point for controlling test attributes per
ADR-005. Users specifying a policy opt into the new system and the macro
should not second-guess the policy.
...
compile-time dependency on `rstest-bdd-harness` so it can emit
fully-qualified trait paths in const assertions. This is acceptable because
