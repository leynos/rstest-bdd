...
1. **Async steps in async scenarios (no harness):** supported. The macro calls
   ...
   wrapper is involved.
2. **Async steps in synchronous scenarios under `TokioHarness`:** rejected at
   runtime. The sync wrapper generated by `generate_sync_wrapper_from_async`
   (`emit.rs`) checks `tokio::runtime::Handle::try_current()` and returns an
   error when a Tokio runtime is already active — which is the case inside
   `TokioHarness`. The fix is either to make the scenario `async fn` (removing
   the harness) or to convert the step to a synchronous function that drives
   async work via `tokio::spawn` / `tokio::spawn_local`.
