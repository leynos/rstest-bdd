...
/// Run a macro implementation inside the `proc_macro_error` entry point so
/// diagnostics can leverage `abort!` helpers while preserving documentation on
/// the exported macro wrappers.
fn run_with_macro_errors<F>(operation: F) -> TokenStream
where
    F: FnOnce() -> TokenStream + UnwindSafe,
{
    entry_point(operation, false)
}
