...
/// Which framework-specific test attribute the policy is expected to emit.
#[derive(Clone, Copy)]
...
fn generate_test_attrs_honours_harness_precedence(
    ...
    #[case] expected: ExpectedAttrs,
) {
    let ExpectedAttrs {
        tokio: expect_tokio_test,
        gpui: expect_gpui_test,
    } = expected;
