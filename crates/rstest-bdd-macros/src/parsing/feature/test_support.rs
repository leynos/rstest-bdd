//! Builders and helpers shared by feature parsing tests.

use super::super::{ParsedStep, ScenarioData, extract_scenario_steps};
...
impl StepBuilder {
    pub(crate) fn new(ty: StepType, value: &str) -> Self {
        Self {
            ty,
    ...
    pub(crate) fn with_keyword(mut self, kw: &str) -> Self {
        self.keyword = Some(kw.to_string());
        self
    }

    pub(crate) fn with_docstring(mut self, doc: &str) -> Self {
        self.docstring = Some(doc.to_string());
        self
    }

    pub(crate) fn with_table(mut self, rows: Vec<Vec<&str>>) -> Self {
...
impl FeatureBuilder {
    ...
    pub(crate) fn with_background(mut self, steps: Vec<Step>) -> Self {
        self.background = Some(steps);
        self
    }

    pub(crate) fn with_scenario(mut self, name: &str, steps: Vec<Step>) -> Self {
        self.scenarios.push((name.to_string(), steps));
        self
    }

    pub(crate) fn build(self) -> gherkin::Feature {
        gherkin::Feature {
...
pub(crate) fn assert_feature_extraction(
