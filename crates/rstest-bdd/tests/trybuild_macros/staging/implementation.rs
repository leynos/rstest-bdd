//! Trybuild staging implementation.
...
pub(crate) fn trybuild_target_directory(workspace_root: &Utf8Path) -> Utf8PathBuf {
...
pub(crate) struct TargetRootSnapshotGuard {
        ...
        for (path, contents) in &self.originals {
            if let Err(error) = crate_dir.write(path.as_std_path(), contents.as_bytes()) {
                tracing::warn!(
                    path = %path,
                    error = %error,
                    "cannot restore trybuild snapshot `{path}`: {error}"
                );
            }
        }
    }
...
pub(crate) fn stage_target_root_snapshots() -> io::Result<TargetRootSnapshotGuard> {
