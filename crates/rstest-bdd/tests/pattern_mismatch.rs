...
use rstest_bdd::{StepContext, StepError, StepKeyword, lookup_step_with_metadata};
...
fn passes_captured_value() {
    ...
    let step_fn = lookup_step_with_metadata(StepKeyword::Given, "number {value:u32}".into())
        .expect("step missing")
        .run;
