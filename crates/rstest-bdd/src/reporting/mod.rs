...
fn reset_dump_seeds_state() {
    ...
    // A failed exchange means a seed run is still in flight, which is exactly
    // the case this guard exists to skip.
    let _previous = state.compare_exchange(2, 0, Ordering::SeqCst, Ordering::SeqCst);
}
