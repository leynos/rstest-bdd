...
execution-error-missing-fixtures = Stap « { $step_pattern } » (gedefinieerd in { $step_location }) vereist fixtures { $required }, maar de volgende ontbreken: { $missing }. Details van gevraagde fixtures: { $missing_requirements }. Beschikbare fixtures van scenario: { $available } { $suggestion } (feature: { $feature_path }, scenario: { $scenario_name })
