...
execution-error-missing-fixtures = Steget "{ $step_pattern }" (definierat vid { $step_location }) kräver fixturer { $required }, men följande saknas: { $missing }. Information om begärda fixtures: { $missing_requirements }. Tillgängliga fixturer från scenariot: { $available } { $has_suggestion ->
    [yes] Välj ett harness-stött scenario så att rstest_bdd_harness_context infogas.
   *[no] {""}
