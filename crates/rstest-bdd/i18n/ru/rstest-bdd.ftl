...
execution-error-missing-fixtures = Шаг «{ $step_pattern }» (определён в { $step_location }) требует фикстуры { $required }, но следующие отсутствуют: { $missing }. Детали запрошенной фикстуры: { $missing_requirements }. Доступные фикстуры из сценария: { $available } { $suggestion } (feature: { $feature_path }, сценарий: { $scenario_name })
