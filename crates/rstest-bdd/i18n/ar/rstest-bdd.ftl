...
execution-error-missing-fixtures = الخطوة « { $step_pattern } » (المعرّفة في { $step_location }) تتطلب التجهيزات { $required }، لكن التالية مفقودة: { $missing }. تفاصيل التثبيتات المطلوبة: { $missing_requirements }. التجهيزات المتاحة من السيناريو: { $available } { $suggestion } (الميزة: { $feature_path }، السيناريو: { $scenario_name })
