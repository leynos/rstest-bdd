...
execution-error-missing-fixtures = L'étape « { $step_pattern } » (définie à { $step_location }) requiert les fixtures { $required }, mais les suivantes sont manquantes : { $missing }. Détails des fixtures demandés : { $missing_requirements }. Fixtures disponibles depuis le scénario : { $available } { $suggestion } (feature : { $feature_path }, scénario : { $scenario_name })
