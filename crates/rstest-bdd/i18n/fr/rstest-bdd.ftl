...
execution-error-missing-fixtures = L'étape « { $step_pattern } » (définie à { $step_location }) requiert les fixtures { $required }, mais les suivantes sont manquantes : { $missing }. Détails des fixtures demandées : { $missing_requirements }. Fixtures disponibles depuis le scénario : { $available } { $has_suggestion ->
    [yes] Sélectionnez un scénario pris en charge par le harness afin que rstest_bdd_harness_context soit inséré.
   *[no] {""}
} (feature : { $feature_path }, scénario : { $scenario_name })
