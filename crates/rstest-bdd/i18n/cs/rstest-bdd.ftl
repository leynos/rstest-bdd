...
execution-error-missing-fixtures = Krok „{ $step_pattern }“ (definován v { $step_location }) vyžaduje fixtury { $required }, ale následující chybí: { $missing }. Requested fixture details: { $missing_requirements }. Dostupné fixtury ze scénáře: { $available } { $suggestion } (feature: { $feature_path }, scénář: { $scenario_name })
