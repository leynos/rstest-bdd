...
execution-error-missing-fixtures = Pasul „{ $step_pattern }” (definit la { $step_location }) necesită fixturi { $required }, dar următoarele lipsesc: { $missing }. Detalii despre fixturile solicitate: { $missing_requirements }. Fixturi disponibile din scenariu: { $available } { $suggestion } (feature: { $feature_path }, scenariu: { $scenario_name })
