...
execution-error-missing-fixtures = Krok « { $step_pattern } » (zdefiniowany w { $step_location }) wymaga fiksur { $required }, ale brakuje: { $missing }. Requested fixture details: { $missing_requirements }. Dostępne fiksury ze scenariusza: { $available } { $suggestion } (feature: { $feature_path }, scenariusz: { $scenario_name })
