...
execution-error-missing-fixtures = گام «{ $step_pattern }» (تعریف‌شده در { $step_location }) به فیکسچرهای { $required } نیاز دارد، اما موارد زیر وجود ندارند: { $missing }. جزئیات فیکسچر درخواست‌شده: { $missing_requirements }. فیکسچرهای موجود از سناریو: { $available } { $suggestion } (feature: { $feature_path }، سناریو: { $scenario_name })
