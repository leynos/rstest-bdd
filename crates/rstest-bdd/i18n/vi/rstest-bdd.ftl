...
execution-error-missing-fixtures = Bước « { $step_pattern } » (định nghĩa tại { $step_location }) yêu cầu fixture { $required }, nhưng các fixture sau bị thiếu: { $missing }. Chi tiết fixture được yêu cầu: { $missing_requirements }. Fixture có sẵn từ kịch bản: { $available } { $suggestion } (tính năng: { $feature_path }, kịch bản: { $scenario_name })
