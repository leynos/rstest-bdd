...
execution-error-missing-fixtures = Trinn "{ $step_pattern }" (definert ved { $step_location }) krever fiksturer { $required }, men følgende mangler: { $missing }. Detaljer om forespurte fixtures: { $missing_requirements }. Tilgjengelige fiksturer fra scenario: { $available } { $suggestion } (funksjon: { $feature_path }, scenario: { $scenario_name })
