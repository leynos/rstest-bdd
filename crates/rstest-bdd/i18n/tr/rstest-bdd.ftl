...
execution-error-missing-fixtures = « { $step_pattern } » adımı ({ $step_location } konumunda tanımlanmış) { $required } fikstürlerini gerektiriyor, ancak şunlar eksik: { $missing }. Requested fixture details: { $missing_requirements }. Senaryodan mevcut fikstürler: { $available } (özellik: { $feature_path }, senaryo: { $scenario_name })
