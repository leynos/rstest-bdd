...
execution-error-missing-fixtures = הצעד "{ $step_pattern }" (מוגדר ב-{ $step_location }) דורש קיבועים { $required }, אך הבאים חסרים: { $missing }. Requested fixture details: { $missing_requirements }. קיבועים זמינים מהתרחיש: { $available } (תכונה: { $feature_path }, תרחיש: { $scenario_name })
