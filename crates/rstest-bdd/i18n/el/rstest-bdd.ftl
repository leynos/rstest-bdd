...
execution-error-missing-fixtures = Το βήμα «{ $step_pattern }» (ορισμένο στο { $step_location }) απαιτεί fixtures { $required }, αλλά τα ακόλουθα λείπουν: { $missing }. Λεπτομέρειες ζητούμενων fixtures: { $missing_requirements }. Διαθέσιμα fixtures από το σενάριο: { $available } { $has_suggestion ->
    [yes] Επιλέξτε ένα σενάριο που υποστηρίζεται από harness ώστε να εισαχθεί το rstest_bdd_harness_context.
   *[no] {""}
} (feature: { $feature_path }, σενάριο: { $scenario_name })
