...
execution-error-missing-fixtures = स्टेप « { $step_pattern } » ({ $step_location } पर परिभाषित) को फ़िक्चर { $required } की आवश्यकता है, लेकिन निम्नलिखित अनुपस्थित हैं: { $missing }। सिनेरियो से उपलब्ध फ़िक्चर: { $available } { $suggestion } (फ़ीचर: { $feature_path }, सिनेरियो: { $scenario_name })
