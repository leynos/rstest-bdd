...
impl<'a> FeatureSource<'a> {
    const fn new(source: &'a str) -> Self { Self(source) }

    const fn as_str(&self) -> &'a str { self.0 }
...
pub fn index_feature_source(
    ...
    index_feature_text(path, FeatureSource::new(normalized.as_ref()))
}
...
fn index_steps_for_container(
    ...
    for step in steps {
        ...
        let docstring = match step.docstring.as_ref() {
            Some(value) => {
                let start_from = table
                    .as_ref()
                    .map_or(step.span.end, |indexed_table| indexed_table.span.end);
