...
#[test]
fn assigns_steps_to_the_nearest_step_library() {
    let source = concat!(
    ...
    let account = index
        .step_definitions
        .iter()
        .find(|step| step.function.name == "account_step")
        .expect("account step");
    let transfer = index
        .step_definitions
        .iter()
        .find(|step| step.function.name == "transfer_step")
        .expect("transfer step");
    assert_eq!(account.library, "accounts");
    assert_eq!(transfer.library, "accounts::transfers");
}
