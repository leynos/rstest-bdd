...
/// Resolve a request URI to a local path with the expected extension.
///
/// Returns `None` — after tracing why — when the URI is not a local file or
/// names a file the handler does not serve. Keeping the guard here leaves the
/// request handlers free of the branching that `debug!` expands into.
///
/// # Examples
///
/// ```ignore
...
/// ```
pub(crate) fn resolve_request_path(uri: &Url, ext: &str, request: &str) -> Option<PathBuf> {
    let Ok(path) = uri.to_file_path() else {
        debug!(%uri, request, "ignoring request for non-file URI");
        return None;
    };
    if !has_extension(&path, ext) {
        debug!(
            ...
            request,
            expected = ext,
            "ignoring request for unrelated file"
        );
        return None;
    }
    Some(path)
