...
fn setup_source_scaffold(root: &Path) -> std::io::Result<(PathBuf, PathBuf, PathBuf)> {
...
fn build_flat_source_with_symlink(
    ...
    for name in file_names {
        fs::write(src.join(name), b"data")?;
    }
    symlink(&target, src.join(symlink_name))?;
    Ok((src, dst))
}
...
fn build_nested_source_with_symlink(
    ...
    symlink(&target, sub.join(symlink_name))?;
    Ok((src, dst))
}
