...
mod tests {
    ...
    /// returned string when the panic payload is an owned `String`.
    #[rstest]
    fn augmented_panic_message_includes_scenario_name_for_string_payload() {
        ...
        let payload: Box<dyn std::any::Any + Send> = Box::new("step panicked".to_string());
        let message = GpuiHarness::augmented_panic_message(payload.as_ref(), &metadata);
        assert!(
            message.contains("A string payload scenario"),
            "expected scenario name in: {message}"
        );
    }

    /// Asserts that `augmented_panic_message` embeds the scenario name in the
    /// returned string when the panic payload is a `&'static str`.
    #[rstest]
    fn augmented_panic_message_includes_scenario_name_for_str_payload() {
        let metadata = ScenarioMetadata::new(
            "tests/features/example.feature",
        ...
        let payload: Box<dyn std::any::Any + Send> = Box::new("step panicked");
        let message = GpuiHarness::augmented_panic_message(payload.as_ref(), &metadata);
        assert!(
            message.contains("A str payload scenario"),
            "expected scenario name in: {message}"
        );
    }

    /// Asserts that `augmented_panic_message` embeds the scenario name in the
    /// returned string when the panic payload is an opaque type that is
    /// neither `String` nor `&str`.
    #[rstest]
    fn augmented_panic_message_includes_scenario_name_for_opaque_payload() {
        #[derive(Debug)]
        struct Opaque;
        let metadata = ScenarioMetadata::new(
            "tests/features/example.feature",
        ...
        let payload: Box<dyn std::any::Any + Send> = Box::new(Opaque);
        let message = GpuiHarness::augmented_panic_message(payload.as_ref(), &metadata);
        assert!(
            message.contains("An opaque payload scenario"),
            "expected scenario name in: {message}"
        );
    }

    /// Asserts that `write_stderr_diagnostic_to` returns an `Err` and does
    /// not panic when the underlying writer always fails with `BrokenPipe`.
    #[rstest]
    fn write_stderr_diagnostic_to_returns_err_on_broken_pipe() {
        ...
        impl Write for BrokenPipeWriter {
            /// Always fails with `BrokenPipe` so the caller observes an
            /// `Err` instead of writing any bytes.
